...
void setup(){                                                  // 
     ...
     sensorA.begin(&sWire);                                    //   Инициируем работу с датчиком объекта sensorA - это акселерометр. Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
     sensorG.begin(&sWire);                                    //   Инициируем работу с датчиком объекта sensorG - это гироскоп.     Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
     sensorM.begin(&sWire);                                    //   Инициируем работу с датчиком объекта sensorM - это магнитометр.  Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
