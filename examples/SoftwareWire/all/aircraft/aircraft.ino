// ПРИМЕР ЧТЕНИЯ КВАТЕРНИОНОВ:                                 // * Строки со звёздочкой являются необязательными.
// Перед определением положения модуля рекомендуется           //   АКСЕЛЕРОМЕТР: измеряет проекцию кажущегося ускорения, которое является геометрической разницей между истинным ускорением и ускорением силы гравитации (g).
// выполнить калибровку см. пример: all/aircraft_calib         //   Если акселерометр неподвижен (истинное ускорение отсутствует), его оси XY параллельны земле, а ось Z направлена вверх, то ...
