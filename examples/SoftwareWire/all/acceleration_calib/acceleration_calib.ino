...
// с предварительной калибровкой датчиков                      //   АКСЕЛЕРОМЕТР: измеряет проекцию кажущегося ускорения, которое является геометрической разницей между истинным ускорением и ускорением силы гравитации (g).
...
//   Выполняем калибровку в неподвижном состоянии:             //
     Serial.println(F("Выполняем калибровку..."));             // *
...
//   Ждём стабилизации данных о положении с фильтра Маджвика:  //
     Serial.println(F("Ждём стабилизации показаний..."));      // *
     uint32_t i = millis();                                    //   Запоминаем время начала ожидания.
     while((millis()-i)<7000){ sensor.read(BMX_M_S); }         //   Обращаемся к функции read(BMX_M_S) в течении 7 секунд.
//   ВАЖНО:                                                    //
...
//   sensor.read(BMX_G  );                                     //   В этом скетче объект sensor создан для работы со всеми датчиками модуля (BMX).
                                                               ...
                                                               //   sensor.read(BMA_M_S  ); читать проекцию кажущегося ускорения с акселерометра в м/с².
                                                               //   sensor.read(BMA_G    ); читать проекцию кажущегося ускорения с акселерометра в g.
...
//   исключив её влияние на кажущееся ускорение.               //
