// ПРИМЕР ЧТЕНИЯ ДАННЫХ АКСЕЛЕРОМЕТРА:                         // * Строки со звёздочкой являются необязательными.
...
void setup(){                                                  // 
     ...
     sensor.begin(&sWire);                                     //   Инициируем работу с акселерометром, так как именно для работы с ним создан объект sensor. Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
...
void loop(){                                                   // 
     sensor.read();                                            //   Функция read() читает данные того датчика, для которого был создан объект sensor.
                                                               ...
                                                               //   sensor.read(BMA_M_S); читать проекцию кажущегося ускорения в м/с² (по умолчанию).
                                                               //   sensor.read(BMA_G  ); читать проекцию кажущегося ускорения в g.
     ...
     Serial.print((String) "X="+sensor.axisX+", ");            //   Выводим проекцию кажущегося ускорения на ось X в м/с².
     Serial.print((String) "Y="+sensor.axisY+", ");            //   Выводим проекцию кажущегося ускорения на ось Y в м/с².
     Serial.print((String) "Z="+sensor.axisZ+" м/с² \r\n");    //   Выводим проекцию кажущегося ускорения на ось Z в м/с².
}