// ПРИМЕР ОТДЕЛЬНОГО ЧТЕНИЯ ВСЕХ ДАТЧИКОВ:                     // * Строки со звёздочкой являются необязательными.
                                                               //   АКСЕЛЕРОМЕТР: измеряет проекцию кажущегося ускорения, которое является геометрической разницей между истинным ускорением и ускорением силы гравитации (g).
...
void setup(){                                                  // 
     ...
     sensorA.begin(&Wire); // &Wire1, &Wire2 ...               //   Инициируем работу с датчиком объекта sensorA - это акселерометр. Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
     sensorG.begin(&Wire); // &Wire1, &Wire2 ...               //   Инициируем работу с датчиком объекта sensorG - это гироскоп.     Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
     sensorM.begin(&Wire); // &Wire1, &Wire2 ...               //   Инициируем работу с датчиком объекта sensorM - это магнитометр.  Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
...
//   Функцию read() можно вызвать с параметром, выбрав единицы измерений выводимых данных. Если параметра нет, то данные выводятся в единицах по умолчанию.
     sensorA.read();                                           //   sensorA.read(BMA_M_S  ); читать проекцию кажущегося ускорения в м/с² (по умолчанию).
                                                               //   sensorA.read(BMA_G    ); читать проекцию кажущегося ускорения в g.
     ...
     Serial.print((String) "X="+sensorA.axisX+", ");           //   Выводим проекцию кажущегося ускорения на ось X в м/с².
     Serial.print((String) "Y="+sensorA.axisY+", ");           //   Выводим проекцию кажущегося ускорения на ось Y в м/с².
     Serial.print((String) "Z="+sensorA.axisZ+" м/с²) ");      //   Выводим проекцию кажущегося ускорения на ось Z в м/с².
