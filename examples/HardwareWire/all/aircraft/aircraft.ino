// ПРИМЕР ЧТЕНИЯ КВАТЕРНИОНОВ:                              // АКСЕЛЕРОМЕТР: измеряет кажущееся угловое ускорение, которое является геометрической разницей между истинным угловым ускорением и ускорением силы гравитации (g).
// Перед определением положения модуля рекомендуется        // Если акселерометр неподвижен (истинное ускорение отсутствует), его оси XY параллельны земле, а ось Z направлена вверх, то ...
// выполнить калибровку см. пример: all/aircraft_calib      // проекция кажущегося ускорения на ось Z = 0 - -g = g. Где 0 это истинное ускорение, а -g сила гравитации направленная в сторону противоположную оси Z.
...
//   while(не_истекло_время){sensor.setFastOffset(BMM);}    // Выполняем калибровку магнитометра после установки нового диапазона измерений или частоты обновления данных в течении указанного времени модуль должен вращаться (см. пример: aircraft_calib).
}                                                           //
                                                            //
void loop(){                                                //
     sensor.read();                                         // Функция read() читает данные того датчика, для которого был создан объект sensor.
                                                            ...
                                                            // Кватернионы вычисляются функции read() только если объект sensor создан для всех датчиков модуля (BMX).
     ...
     Serial.print(",");                                     //
     Serial.print(sensor.q4);                               // Выводим кватернион q4.
