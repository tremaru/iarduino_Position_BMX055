// ПРИМЕР ЧТЕНИЯ ДАННЫХ ГИРОСКОПА:                          // ГИРОСКОП: измеряет угловую скорость вокруг собственных осей, он определяет воздействие момента внешней силы.
// с предварительной калибровкой датчика                    // Эти данные можно использовать, например, для включения режима фиксации жесткого диска в случае падения, удара или резкого перемещения ноутбука.
                                                            //
...
void setup(){                                               // 
     ...
     sensor.begin(&Wire); // &Wire1, &Wire2 ...             // Инициируем работу с гироскопом, так как именно для работы с ним создан объект sensor. Указав ссылку на объект для работы с шиной I2C на которой находится модуль (по умолчанию &Wire).
...
//   Выводим подсказку по калибровке:                       // Чем выше диапазон и частота, тем ниже точность.
     ...
     Serial.println("Калибровка начнётся через 5 сек.");    //
     delay(5000);                                           //
