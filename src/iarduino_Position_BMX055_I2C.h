...
class iarduino_I2C_Select: public iarduino_I2C_VirtualSelect{													//
	...
	//	Функции переинициации ранее выбранной шины I2C:															//
		...
		void begin(uint8_t adr){																				//
			#if defined(I2C_HW_includes)																		//
				if(flgI2CType==1){ (*(TwoWire*    )objI2C).begin(adr); }										//
			#endif																								//
			#if defined(I2C_SW_includes)																		//
	...
	//	Функция получения номера вывода SCL:																	//
		uint8_t getPinSCL(void){																				//
			...
			return (uint8_t)scl;																				//
																												...
																												//
	//	Функции изменения таймаута ожидания ведомого в мкс (для программной шины):								//
		...
		void setWireTimeout(void){	//	использовать время таймаута по умолчанию.								//
				...
				if(flgI2CType==2){ (*(SoftTwoWire*)objI2C).setWireTimeout(); }									//
																												...
																												//
	//	Функция изменения скорости передачи данных:																//
		void setClock(uint32_t speed){																			//
			#if defined(I2C_HW_includes)																		//
				if(flgI2CType==1){ (*(TwoWire*    )objI2C).setClock(speed); }									//
			#endif																								//
			#if defined(I2C_SW_includes)																		//
	...
	//	Функция указания пользовательской функции обработки получения данных ведомым от мастера:				//
		void onReceive(void (*func)(int )){																		//
				...
				if(flgI2CType==2){ (*(SoftTwoWire*)objI2C).onReceive(func); }									//
																												...
																												//
	//	Функция указания пользовательской функции обработки получения запроса ведомым от мастера:				//
		void onRequest(void (*func)(void)){																		//
			#if defined(I2C_HW_includes)																		//
				if(flgI2CType==1){ (*(TwoWire*    )objI2C).onRequest(func); }									//
			#endif																								//
			#if defined(I2C_SW_includes)																		//
