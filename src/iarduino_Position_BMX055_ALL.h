//	Акселерометр, Гироскоп и Магнитометр	//	Это три датчика, показания которых можно использовать для определения абсолютной ориентации устройства в пространстве
											//	Данные проекции кажущегося ускорения, угловой скорости и магнитной индукции считываются с акселерометра, гироскопа и магнитометра, и являются входными данными для фильтра Маджвика, Махони, Калмана, или др.
...
#define	BMX_M_S					6			//	Аргумент функции read() указывающий выводить данные в м/с²         истинного ускорения (не путать с проекцией кажущегося ускорения акселерометра)
#define	BMX_G					7			//	Аргумент функции read() указывающий выводить данные в количестве g истинного ускорения (не путать с проекцией кажущегося ускорения акселерометра)
											//	
...
//		Чтение данных из датчиков:																								//
		bool	read(uint8_t thisMera){																							//	Аргумент: мера измерения читаемых данных (0 - градусы / 1 - радианы)
					bool f, result=true;																						//
					#ifndef BMX055_DISABLE_BMA																					//	Если используется акселерометр
					f = objBMA -> read(BMA_G);		/* кажущееся ускорение в g (количестве ускорений свободного падения) */		//	Вызываем функцию чтения данных акселерометра, которые запишутся в переменные BMAx, BMAy, BMAz, BMAt. Если данные не будут прочтены, то в переменных останутся предыдущие значения
					if( !f ){ result=false; }																					//
					#endif																										//
					#ifndef BMX055_DISABLE_BMG																					//	Если используется гироскоп
				...
				//	Если требуется получить истинное ускорение:																	//
					if( thisMera==BMX_M_S || thisMera==BMX_G ){																	//	Истинное ускорение = вектор кажущегося ускорения акселерометра - вектор ускорения свободного падения (g)
						...
						float gX = -cos(angleX);																				//	Вычисляем gX - проекцию вектора g на ось axisX (-1...0...+1).
						float gY =  sin(angleX)*cos(angleY);																	//	Вычисляем gY - проекцию вектора g на ось axisY (-1...0...+1).
						float gZ =  sin(angleX)*sin(angleY);																	//	Вычисляем gZ - проекцию вектора g на ось axisZ (-1...0...+1).
						if( thisMera==BMX_M_S ){ j=9.81f; }																		//
						*axisX = (BMAx-gX)*j;																					//	Вычисляем разность между (проекциями векторов на ось axisX) кажущегося ускорения (BMAx) и ускорения свободного падения (gX). Для BMX_M_S переводим g в м/с², умножая результат на 9.81 м/с²
						*axisY = (BMAy-gY)*j;																					//	Вычисляем разность между (проекциями векторов на ось axisY) кажущегося ускорения (BMAy) и ускорения свободного падения (gY). Для BMX_M_S переводим g в м/с², умножая результат на 9.81 м/с²
						*axisZ = (BMAz-gZ)*j;																					//	Вычисляем разность между (проекциями векторов на ось axisZ) кажущегося ускорения (BMAz) и ускорения свободного падения (gX). Для BMX_M_S переводим g в м/с², умножая результат на 9.81 м/с²
					}																											//	
					return result;																								//
		}																														//
																																//
